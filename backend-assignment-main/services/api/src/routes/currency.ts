...
import { addCurrency, deleteCurrency, ListCurrency } from "./../controllers/currencyController";
...
routes.get("/currency/:code", async (req, res) => {
    function requestApi() {
        return new Promise<JSON>((resolve, rejects) => {
            https.get(`https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=${req.params.code}&to_currency=EUR&apikey=J3E7KVFNXL5VP9BE`, res => {
